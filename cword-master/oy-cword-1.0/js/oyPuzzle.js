...
oyCrosswordPuzzle.prototype.init = function(){ 
	...
	// URL that is visits after clicking "Copyright (2006) Pavel Simakov" in the bottom
	document.getElementById("oygCopyright").onclick = function(){
		oThis.menu.leaveGameEarly(oThis.publisherURL); //URL
		return false; 
	}	
  
	//Upper right corner "whereto.gif" image click part
	...
	target.innerHTML = trackAction + '<a id="oygHeaderMenuBtn" href=""><img style="padding: 4px;" src="' + this.appHome + '/img/whereto.gif" border="0" alt="Leave Game"></a>';
	 ...
	 // URL that is visits after clicking "whereto.gif"  image in the upper right corner
	document.getElementById("oygHeaderMenuBtn").onclick = function(){
		oThis.menu.leaveGameEarly(oThis.leaveGameURL); //URL
		return false; 
	}	
...
oyCrosswordPuzzle.prototype.render = function(){
	...
	var target = document.getElementById("oygListH"); // Across Clue part
	target.innerHTML = "";			
	target.className = "oyPanelDivHidden";
	
	var target = document.getElementById("oygListV"); //Down Clue Part
	target.innerHTML = "";
	target.className = "oyPanelDivHidden";		
}
...
oyCrosswordPuzzle.prototype.fillIn = function(cell, x, y, i, idx, dir){
	if (i == 0){   // Provide number in cell from root folder image.  
